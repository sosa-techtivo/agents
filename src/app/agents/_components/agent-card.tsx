...
import { countPassedTestCases } from "@/lib/agents/summary";
...
export function AgentCard({
  ...
  const passedCases = lastRun ? countPassedTestCases(lastRun) : 0;

  return (
    <div className={`${cardBase} border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950`}>
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-medium text-zinc-900 dark:text-zinc-50">{name}</h2>
          <p className="text-sm text-zinc-500 dark:text-zinc-400">Project: {project}</p>
        </div>
        <StatusBadge label="Ready" tone="ready" />
      </div>
      <dl className="grid grid-cols-2 gap-y-1 text-sm text-zinc-600 dark:text-zinc-400">
        <dt>Last run</dt>
        <dd className="text-right">{lastRun ? formatDateTime(lastRun.startedAt) : "Never"}</dd>
        <dt>Last result</dt>
        <dd className="text-right">
          {lastRun
            ? `${lastRun.status === "passed" ? "Passed" : "Failed"} · ${passedCases}/${lastRun.testCases.length} test cases`
            : "—"}
        </dd>
      </dl>
      <Link
